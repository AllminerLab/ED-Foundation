...
def construct_text(row: pd.Series, template: str) -> str:
    ...
    if template == "triage_cn":
        return (
            f"主诉: {value_or_empty(row, 'chief_complaint')}. "
            f"年龄: {value_or_empty(row, 'age')}. 性别: {value_or_empty(row, 'sex')}. "
            f"来院方式: {value_or_empty(row, 'arrival_mode')}. "
            f"早期生命体征: {format_vitals(row)}."
        )
    ...
    if template == "clinical_text_cn":
        return (
            f"主诉: {value_or_empty(row, 'chief_complaint')}. "
            f"现病史: {value_or_empty(row, 'history_present_illness')}. "
            f"既往史: {value_or_empty(row, 'past_medical_history')}. "
            f"初步诊断: {value_or_empty(row, 'primary_diagnosis')}. "
            f"年龄: {value_or_empty(row, 'age')}. 性别: {value_or_empty(row, 'sex')}. "
            f"早期生命体征: {format_vitals(row)}."
        )

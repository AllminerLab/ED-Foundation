#!/usr/bin/env python3
"""
SYSMH-ED-MD training script.
Text-only multi-task ED medical decision modeling.
Tasks: 1) imaging exam, 2) lab tests, 3) observation, 4) specialist consultation.
"""
...
class SYSMHMDDataset(Dataset):
    """SYSMH-ED-MD dataset for four medical decision tasks."""
...
def main():
    ...
    print("="*80)
    print("SYSMH-ED-MD training - four medical decision tasks")
    print("="*80 + "\n")
    ...
    for epoch in range(args.num_epochs):
        ...
        if avg_f1 > best_avg_f1:
            ...
            print(f"  Saved best model (average F1: {avg_f1:.4f})")
    
    with open(os.path.join(args.save_dir, 'training_history.json'), 'w') as f:
        json.dump(history, f, indent=2)
    
    print(f"\nTraining complete. Best average F1: {best_avg_f1:.4f}")
